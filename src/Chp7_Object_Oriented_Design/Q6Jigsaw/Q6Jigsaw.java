...
public class Q6Jigsaw {
    ...
    public static Edge[] createEdges(Piece[][] puzzle, int column, int row) {
        ...
        /* Get left edge */
        Edge left = column == 0 ? new Edge(Shape.FLAT, key + "h|e") : puzzle[row][column - 1]
                .getEdgeWithOrientation(Orientation.RIGHT).createMatchingEdge();

        /* Get top edge */
        Edge top = row == 0 ? new Edge(Shape.FLAT, key + "v|e") : puzzle[row - 1][column]
                .getEdgeWithOrientation(Orientation.BOTTOM).createMatchingEdge();
